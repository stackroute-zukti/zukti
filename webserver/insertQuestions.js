...
let driver = neo4j.driver('bolt://192.168.1.120', neo4j.auth.basic('neo4j', 'neo4js'));
        ...
        if (lineNumber !== 1 && linearr[3] !== null) {
            if (linearr[8] !== '') {
                let query = `match (n:${linearr[0]} {id:${linearr[1]}})
                 ...
                 merge (i:image{value:'${linearr[8]}'}) merge (q)-[:answer{rating:0}]->(i)`;
                ...
                if (linearr[5].length > 1) {
                    let query = `match (n:${linearr[0]} {id:${linearr[1]}})
                    merge (q:question{value:'${linearr[4]}'}) merge (q)-[:${linearr[3]}]->(n)
                    merge (t:text{value:'${linearr[5]}'}) merge (q)-[:answer{rating:0}]->(t)`;
                    session.run(query).then(function(result) {
                        session.close();
                        driver.close();
                    }).catch(function(error) {
                ...
                if (linearr[6].length > 1) {
                    let query = `match (n:${linearr[0]} {id:${linearr[1]}})
                    merge (q:question{value:'${linearr[4]}'}) merge (q)-[:${linearr[3]}]->(n)
                    merge (b:blog{value:'${linearr[6]}'}) merge (q)-[:answer{rating:0}]->(b)`;
                    session.run(query).then(function(result) {
                        session.close();
                        driver.close();
                    }).catch(function(error) {
                ...
                if (linearr[7].length > 1) {
                    let query = `match (n:${linearr[0]} {id:${linearr[1]}})
                    merge (q:question{value:'${linearr[4]}'})   merge (q)-[:${linearr[3]}]->(n)
                    merge (v:video{value:'${linearr[7]}'}) merge (q)-[:answer{rating:0}]->(v)`;
                    session.run(query).then(function(result) {
                        session.close();
                        driver.close();
                    }).catch(function(error) {
