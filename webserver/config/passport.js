...
// @Mayanka: Check if user has been abusive and redirect
            ...
            if (user) {
                ...
                User.findOne({ 'local.email': email }, function(err, userDatas)
                {
                    if(err)
                    {
                        return done(err);
                    ...
                    {
                        User.findOne({ 'google.email': email }, function(err, googleData)
                        {
                            if(err)
                            {
                                return done(err);
...
process.nextTick(function() {
    ...
    // @Mayanka: Check if user has been abusive and redirect
            ...
            if (user) {
                ...
                var email = (profile.emails[0].value || '').toLowerCase()
                User.findOne({ 'local.email': email }, function(err, userData)
                {
                    if(err)
                    {
                        return done(err);
                    ...
                    {
                        User.findOne({ 'facebook.email': email }, function(err, fbData)
                        {
                            if(err)
                            {
                                return done(err);
