module.exports = function (question) {
	...
	let Spellchecker = require(__dirname + '/../' + 'dict/hunspell-spellchecker');
	let fs = require('fs');
	let spellchecker = new Spellchecker();
//  @Mayanka :importing the dictionary files
	let DICT = spellchecker.parse({
		aff: fs.readFileSync(__dirname + '/../' + 'dict/en_US.aff'),
		dic: fs.readFileSync(__dirname + '/../' + 'dict/en_US.dic')});
	let tokenizer = new natural.WordTokenizer();
	//  @Mayanka : importing the file having domain related data
	let text = fs.readFileSync(__dirname + '/../' + 'corpus/lotsofwords.txt', 'utf-8');
