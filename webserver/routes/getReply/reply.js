...
consumerAddUserMicroservice.client({pin:'role:questionprocessing, cmd:process', host: '127.0.0.1', port: 3000});
...
router.post('/askQuestion', function(req, res) {
    ...
    if (abusePresent == true) {
        ...
        let count = 0;
        let newQuestion = '';
        // @Mayanka: if returned string from spellcheck contains 'undefined', send a default message to the user
        if(spellResponse.question.includes('undefined')) {
          ...
          res.json({isUnAnswered: true, answerObj: resultArray});
            }
        else{
        ...
        let finalCallBack = function() {
          // microservice
        let query = {};
        let role = 'questionprocessing';
        let cmd = 'process';
        let lowerCaseNewQuestion = newQuestion.toLowerCase();

        logger.debug('newQuestion: ', lowerCaseNewQuestion);
        logger.debug('intentLexicon: ', intentLexicon);
        logger.debug('keywordLexicon: ', keywordLexicon);
        logger.debug('typeLexicon: ', typeLexicon);

        consumerAddUserMicroservice.act(
          {role, cmd},
          {lowerCaseNewQuestion, intentLexicon, keywordLexicon, typeLexicon},
          function(err, answer) {
            logger.debug('response recieved from seneca...');
            if(err) {
              logger.debug('seneca error: ', err);
            } else {
              query = answer;
              logger.debug('seneca answer: ', answer);
              logger.debug('seneca query: ', query);
                  ...
                  if (isKeywordFound) {
                      ...
                      if(matchingConcepts.length == 1 && intents.length > 0) {
                        ...
                        // suggestionConcepts(matchingConcepts, suggestionCallback);
                            ...
                            getKeywordResponse(matchingConcepts, email, types, sendResponse, otherDomainResponse, spellResponse.flag, spellResponse.question,answerFoundCallback, noAnswerFoundCallback);
                        } else {
                        suggestionConcepts(matchingConcepts, suggestionCallback);
                      }
                  } else {
                    ...
                    getKeywordResponse(keywords, email, types, sendResponse, otherDomainResponse, spellResponse.flag, spellResponse.question, answerFoundCallback, noAnswerFoundCallback);
                    // @vibakar: adding keyword to redis
                    addKeywordToRedis(username, keywords[0], undefined);
                } else {
