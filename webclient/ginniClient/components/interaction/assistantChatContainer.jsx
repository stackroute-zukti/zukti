...
export default class AssistantChatContainer extends React.Component {
            ...
            Axios.get('/retriveChat').then((response) => {
                if (response.data) {
                  ...
                  if(response.data.chats.length === 0) {
                    ...
                    let message = `Hi! I'm Zukti and I'll try to answer your queries on ${domain}.`;
                    ...
                    response.data.chats.forEach((chat, index) => {
                        ...
                        this.state.messages.push(
                            <div key = {'b'+index} ref = {(ref) => this['_div' + length] = ref}>
                        ...
                        if (chat.isUnAnswered) {
                            chat.answerObj.forEach((answer, index) => {
                                let length = this.state.messages.length;
                                this.state.messages.push(
                                    <div key = {'c'+index} ref={(ref) => this['_div' + length] = ref}>
    ...
    pushGinniMessages(ginniReply, loadingDots) {
        ...
        ginniReply.forEach((reply, index) => {
            let length = this.state.messages.length;
            let displayItem = (
                <div key = {'a'+index} ref={(ref) => this['_div' + length] = ref} key={length}>
                    {reply}
                </div>
            );
